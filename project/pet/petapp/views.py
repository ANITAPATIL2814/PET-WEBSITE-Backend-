...
def index(request):
    ...
    p=Pet.objects.filter(is_active=True)
    context['products']=p
...
def catfilter(request,cv): #cv will be anything when we want to pass number we need variable here we use cv
    ...
    p=Pet.objects.filter(q1&q2) # to check both condition
    # to pass take dict
    context={}
    context['products']=p
    return render(request,'index.html',context)
...
def addtocart(request,pid):
    ...
    q1=Q(uid=u[0]) 
    q2=Q(pid=p[0])
    c=petcart.objects.filter(q1 and q2)
    n=len(c)
    print(n) 
    context={}
    context['products']=p
    if n==1: 
        ...
        c=petcart.objects.create(uid=u[0],pid=p[0])
        c.save()
        # for sucess msg
...
def viewcart(request):
    if request.user.is_authenticated:
        c=petcart.objects.filter(uid=request.user.id)
...
def remove(request,cid):
    c=petcart.objects.filter(id=cid)
    c.delete()
    return redirect('/viewcart')

def updateqty(request,qv,cid):# quantity value, cid : id for cart
    c=petcart.objects.filter(id=cid) # fetch all data product
    if qv =='1': 
        t=c[0].qty+1
        c.update(qty=t)
...
def placeorder(request):
    ...
    c=petcart.objects.filter(uid=userid) #to fetch all records of userid
...
def makepayment(request):
    ...
    DATA = {
    "amount": s, #becz our amt is stored in s
    "currency": "INR",
    "receipt": oid,
    "notes": {
        "key1": "value3",
